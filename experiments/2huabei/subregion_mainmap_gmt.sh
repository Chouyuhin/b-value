gmt begin huabei_basemap pdf

    # --- 1. 设置基础底图框 ---
    gmt set MAP_FRAME_TYPE plain FONT_ANNOT_PRIMARY 10p,Helvetica,black
    gmt basemap -R110/117/34/42 -JM15c -Baf -BWSen
    # --- 2. 绘制地形底图 ---
    gmt makecpt -Cgray -G0.99/1 -T-10000/10000 -D
    gmt grdimage @earth_relief_01s -R110/117/34/42 -I+a315+nt0.5 -C 

    # --- 3. 绘制地震点 ---
    EQ_FILE="/Users/chouyuhin/_Harvard/b-value/b_value/experiments/2huabei/region2_huabei.csv"
    gmt makecpt -C'#274753,#297270,#299d8f,#8ab07c,#e7c66b,#f3a361,#e66d50' -T1970/2023
    
    awk -F, 'NR>1 && $11>0 && $8>=110 && $8<=117 && $7>=34 && $7<=42 {print $8, $7, $1}' \
    "$EQ_FILE" | sort -k3n | \
