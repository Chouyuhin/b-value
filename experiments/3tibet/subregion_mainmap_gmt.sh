gmt begin tibet_basemap pdf
    ...
    gmt grdimage @earth_relief_03s -R73/90/35/45 -I+a315+nt0.5 -C 
    # --- 3. 绘制地震点 ---
    EQ_FILE="/Users/chouyuhin/_Harvard/b-value/b_value/experiments/3tibet/region3_tibet.csv"
    gmt makecpt -C'#274753,#297270,#299d8f,#8ab07c,#e7c66b,#f3a361,#e66d50' -T1970/2023
    
    awk -F, 'NR>1 && $11>0 && $8>=73 && $8<=90 && $7>=35 && $7<=45 {print $8, $7, $1}' \
    "$EQ_FILE" | sort -k3n | \
    ...
    # 6.1 画五角星 (黄色)
    awk -F, 'NR>1 {print $0}' "$EQ_FILE" | sort -t, -k11nr | head -n 5 | \
    awk -F, '{print $8, $7}' | \
